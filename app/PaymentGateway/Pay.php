...
class Pay extends Payment
{
    ...
    {
        $api = 'test';
        $amount = $amounts['paying_amount'] * 10; // convert toman to rial
//        $mobile = "شماره موبایل";
    ...
    public function verify($token, $status)
    {
        ...
        if (isset($result->status)) {
            if ($result->status == 1) {
                ...
                \Cart::clear();
                return ['success' => 'پرداخت با موفقیت انجام شد. شماره ی تراکنش: ' . $result->transId];
            } else {
                return ['error' => 'پرداخت با خطا مواجه شد. شماره ی وضعیت: ' . $result->status];
            }
        } else {
            if ($status == 0) {
                return ['error' => 'پرداخت با خطا مواجه شد. شماره ی وضعیت: ' . $status];
            }
        }
    }
